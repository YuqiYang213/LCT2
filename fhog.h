...
float* fhog(float* I,int height,int width,int channel,int *h,int *w,int *d,int binSize = 8,int nOrients = 9,float clip=0.2f,bool crop = false);
cv::Mat fhog(const cv::Mat& input, int binSize = 8,int nOrients = 9,float clip=0.2f,bool crop = false);
